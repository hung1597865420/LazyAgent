...
Hệ thống Agent Harness được xây dựng để làm "hội đồng cố vấn" hỗ trợ cho AI coder chính (**Claude Code**, **Gemini** trên Antigravity IDE). Nó cung cấp **67 MCP tools** tích hợp trực tiếp thông qua 12 AI agents chạy trên Azure AI Foundry; Auto-Pilot có thể tự gọi các scanner/reviewer phù hợp sau edit.
...
### 2.6. Danh mục đủ 67 MCP tools và cần chuẩn bị gì
...
Kết quả: tất cả pass. `smoke_test.py` xác nhận MCP registry có 67 tools, `goal_autopilot status`, `goal_supervisor`, `goal_runner` dry-run, 5 gap tools mới, và các nhánh `prod_readiness_gate`:
...
Nếu `ask_codebase` vẫn timeout 300s trong client hiện tại, gần như chắc là MCP process cũ chưa restart. Chạy lại client hoặc restart MCP server, rồi gọi `goal_supervisor`/`list_tools` để xác nhận thấy 67 tools.
<!-- /harness-current-operations -->
...
|---|---|---|
| MCP server | Cho agent chính gọi được 67 tools | `mcp_server.py` qua MCP config |
...
Cursor/Windsurf/generic MCP clients dùng cùng server command/path ở trên. Nếu một agent chính không hỗ trợ MCP thì không gọi trực tiếp được 67 tools; cần wrapper/bridge riêng.

#### Memory/rules tối thiểu cho agent khác

Không cần copy schema của 67 tools vào rules file. Tool schema nằm trong `mcp_server.py -> list_tools()`. Rules chỉ cần policy gọi tool:
...
Nếu agent chính không support MCP, nó không thể gọi trực tiếp 67 tools. Có 3 hướng:
...
### Kỹ thuật 22: Phân Vai 12 Model — 67 MCP Tools, Auto-Pilot + Auto-Watch
...
Hệ thống có **67 MCP tools**: goal autopilot, goal supervisor, direct `goal_runner`, `prod_readiness_gate`, 7 tool gọi LLM, 55 static/static-first analyzers, và `auto_trigger` để tự fan-out các tool phù hợp. Mặc định Auto-Pilot bật `HARNESS_STATIC_LLM=1`, nên các static scanner vẫn chạy phân tích tĩnh trước rồi gọi Azure để enrich/triage khi tool hỗ trợ.
...
`smoke_test.py` kiểm 67 MCP tools, resources/templates handshake, lazy rules auto-merge stamp/idempotency, sandbox Windows + `returncode`, runtime workspace cho `_run_cmd_safe`, benchmark subprocess, Unicode-safe fix/debug output, Auto-Pilot trigger, Direct goal runner dry-run, Production readiness gate (`mode`, `since_commit`, `ask_user`, `fix_required`), 5 gap tools mới ở `mode=safe`, Auto-Watch detect/ignore/lock/log-redaction, ask_codebase JSON unwrap + fallback citation, goal supervisor enum, devops/security scanners, swarm state machine, và các tool quality. Scratch file chạy trong `.harness_smoke/` và tự cleanup; `doc_sync` chạy trên workspace tạm nên không append README thật.
