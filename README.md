# Hướng Dẫn Cài Đặt và Khai Thác Kỹ Thuật Trong Agent Harness để User làm càng ít càng tốt
...
> **Lưu ý cực quan trọng:** nhớ chọn profile trước khi chạy (`harness-toggle.bat` hoặc `harness-full-setup.bat --profile ...`). Đừng để nhầm `heavy/max` rồi cắm máy chạy tự động cả ngày, không là bay sổ đỏ vì token/quota.

---

## 1. Tổng Quan Kiến Trúc & Phân Vai Agent

Hệ thống Agent Harness được xây dựng để làm "hội đồng cố vấn" hỗ trợ cho AI coder chính (**Claude Code**, **Gemini** trên Antigravity IDE). Runtime hiện đăng ký **83 MCP tools**: core LLM tools, goal autopilot, lesson memory, Hallmark/Spec Kit bridges, OfficeCLI bridge, scope-creep guard, quota reminder và static/static-first analyzers; Auto-Pilot có thể tự gọi scanner/reviewer phù hợp sau edit.
...
MCP Server (agent-harness)                         MCP Server (agent-harness)
  │                                                    │
  ├─► panel_review     (Reviewer+Security+Tester)      ├─► consult          (Analyzer — Sonnet)
...
| :--- | :--- | :--- |
| **Manager** | `ag/gemini-3-flash-agent` | Agent manager mặc định cho `ask_codebase`; chain fallback sang Sonnet khi cần context/judgment sâu hơn. |
| **Synthesizer** | `ag/gemini-3-flash-agent` | Fast JSON synthesis/merge cho `panel_review` code findings. |
| **Analyzer** | `ag/claude-sonnet-4-6` | Đề xuất giải pháp kiến trúc và phân tích trade-offs (concurrency, performance). |
| **Code A** | `ag/gemini-3-flash-agent` | Code agent thứ nhất, code-focused, nhanh và tiết kiệm hơn Sonnet. |
| **Code B** | `ag/claude-sonnet-4-6` | Code agent thứ hai, cố ý chọn cách cài đặt khác biệt (alternative design) để so sánh. |
| **Reviewer** | `ag/gemini-3-flash-agent` | Phân tích chất lượng mã nguồn, phát hiện code smells, bugs và logic gaps. |
| **Tester** | `ag/gemini-3-flash-agent` | **Adversarial devil's advocate** — tìm race conditions, hidden assumptions và edge cases mà Reviewer + Security bỏ sót. Mỗi finding kèm input/scenario cụ thể làm code fail. |
| **Security** | `ag/claude-sonnet-4-6` | Dò quét lỗ hổng bảo mật (Injection, XSS, Secret exposure) có kèm attack vector. |
| **Integrity** | `ag/claude-sonnet-4-6` | **Stage 2 của panel** — chạy sau 3 reviewer: (1) tìm race condition/missing transaction/partial failure gap; (2) synthesize + dedupe toàn bộ findings. 25.77M TPM. |
| **Scanner** | `ag/gemini-3-flash-agent` | Static/code-assist scan cho metric/syntactic tools như `dead_code_scanner`, `performance_regression_detector`. |
| **Debugger** | `ag/gemini-3-flash-agent` | Tiếp nhận mã lỗi hoặc trace log, phân tích root cause và tạo file patch (`.diff`). |
| **Worker** | `ag/gemini-3.5-flash-extra-low` | Tác vụ không phải code: tổng hợp tài liệu, PR/changelog draft, mock data, docstring, boilerplate nhẹ. |

Policy hiện tại: non-code/docs dùng `ag/gemini-3.5-flash-extra-low`; code thường, manager, reviewer, tester, scanner, debugger dùng `ag/gemini-3-flash-agent`; security/integrity/architecture và code deep fallback dùng `ag/claude-sonnet-4-6`.
...
MODEL_MANAGER=ag/gemini-3-flash-agent
MODEL_SYNTHESIZER=ag/gemini-3-flash-agent
MODEL_ANALYZER=ag/claude-sonnet-4-6
MODEL_CODE_A=ag/gemini-3-flash-agent
MODEL_CODE_B=ag/claude-sonnet-4-6
MODEL_REVIEWER=ag/gemini-3-flash-agent
MODEL_TESTER=ag/gemini-3-flash-agent
MODEL_SECURITY=ag/claude-sonnet-4-6
MODEL_INTEGRITY=ag/claude-sonnet-4-6
MODEL_SCANNER=ag/gemini-3-flash-agent
MODEL_DEBUGGER=ag/gemini-3-flash-agent
MODEL_WORKER=ag/gemini-3.5-flash-extra-low

# Fallback khi primary model bị timeout/rate-limit.
SPARE_MODELS=ag/claude-sonnet-4-6,ag/gemini-3-flash-agent,ag/gemini-3.5-flash-extra-low
HARNESS_KNOWN_DEPLOYMENTS=ag/claude-sonnet-4-6,ag/gemini-3-flash-agent,ag/gemini-3.5-flash-extra-low

# ask_codebase fast path.
HARNESS_ASK_CODEBASE_MODEL_CHAIN=ag/gemini-3-flash-agent,ag/claude-sonnet-4-6
...
### 2.2.1. Hallmark/Spec Kit đã chưng cất vào Auto-Pilot

Harness không vendor nguyên repo Hallmark hay Spec Kit vào core. Thay vào đó `tools/integrations.py` rút luật vận hành của hai repo thành route tĩnh, expose qua MCP tool `integration_router`, thêm bridge thật `hallmark_bridge`/`speckit_bridge`, tự đính route vào response của `auto_trigger`, và tự bơm guidance vào prompt agent ngoài của `goal_runner`.
...
- **Profile gate:** profile `off` chỉ cho bridge chạy read-only/status/preflight/snapshot; không tự gọi Hallmark/Spec Kit LLM workflow, không tự init/scaffold Spec Kit, không gọi `goal_runner`.
...
| FinOps + quota reminder | SQLite file `.harness_finops.db` tự tạo, optional 9Router dashboard API | `finops_stats`, `router_quota_status`; thống kê token/cost và nhắc quota còn lại, không chặn request |
| Web dashboard tùy chọn | `python server.py` | Mở `http://localhost:8000` |

### 2.6. Danh mục đủ 83 MCP tools và cần chuẩn bị gì
...
| Harness ops | `harness_doctor`, `context_auditor`, `ask_codebase_health`, `goal_runner_control`, `run_ledger`, `policy_profile`, `agent_adapters`, `benchmark_runner`, `patch_safety_check`, `integration_router`, `hallmark_bridge`, `speckit_bridge`, `scope_creep_detector`, `office_bridge` | Git, optional agent CLI, optional `specify` CLI, optional `officecli` | Self-check, context budget audit, ask_codebase preflight, runner resume/status, audit ledger, profiles, adapters, benchmark dry-run, isolated patch check, Hallmark preflight, Spec Kit status/init/scaffold, scope drift, Office doc inspect/validate |
...
| Incident/intel | `incident_responder`, `telemetry_debugger`, `git_archaeologist`, `finops_stats`, `router_quota_status`, `harness_trace_viewer` | Logs/stack traces, Git blame, `.harness_finops.db`, optional 9Router `/api/usage/*` | Incident triage, stack trace patch hint, why code changed, cost/token stats, quota reminder, harness trace/bottleneck view |
...
Kết quả: tất cả pass. `smoke_test.py` xác nhận MCP registry có 83 tools, `goal_autopilot status`, `goal_supervisor`, `goal_runner` dry-run, ops tools, distilled integration bridges, OfficeCLI/scope guard bridges, quota reminder, 5 gap tools mới, và các nhánh `prod_readiness_gate`:
...
```text
HARNESS_ASK_CODEBASE_MODEL_CHAIN=ag/gemini-3-flash-agent,ag/claude-sonnet-4-6,ag/claude-sonnet-4-6
...
- `HARNESS_ASK_CODEBASE_MODEL_CHAIN` là thứ tự 9Router model thử cho `ask_codebase`; default tránh `ag/claude-sonnet-4-6` vì probe thực tế hay timeout với context lớn. Có thể override bằng `HARNESS_ASK_CODEBASE_MODEL_CHAIN` hoặc `HARNESS_ASK_CODEBASE_MODEL`.
...
Nếu `ask_codebase` vẫn báo model cũ như `ag/claude-sonnet-4-6` và không có `model_attempts`, MCP process hiện tại chưa nạp bản mới. Sau lần restart/nạp mới, `mcp_server.py` tự hot-reload `support_tools` + `tools/*` khi file đổi, nên các update tiếp theo không cần chạy lại settings thủ công.
<!-- /harness-current-operations -->
...
| `panel_review` kẹt stage synthesize | Synthesizer model timeout/retry hoặc context quá lớn | Synthesizer mặc định dùng `ag/gemini-3-flash-agent` thay vì pro; panel reviewer tắt retry/spare nội bộ; Integrity synthesize cap bằng `HARNESS_PANEL_INTEGRITY_TIMEOUT` (default 75s), fail thì trả `degraded: true` + local dedup |
...
|---|---|---|
| MCP server | Cho agent chính gọi được 83 tools | `mcp_server.py` qua MCP config |
...
Cursor/Windsurf/generic MCP clients dùng cùng server command/path ở trên. Nếu một agent chính không hỗ trợ MCP thì không gọi trực tiếp được 83 tools; cần wrapper/bridge riêng.

#### Memory/rules tối thiểu cho agent khác

Không cần copy schema của 83 tools vào rules file. Tool schema nằm trong `mcp_server.py -> list_tools()`. Rules chỉ cần policy gọi tool:
...
Nếu agent chính không support MCP, nó không thể gọi trực tiếp 83 tools. Có 3 hướng:
...
2. Các mô hình thường (Gemini High, Sonnet, GPT thường) chạy **Chat Completions API** (`http://localhost:20128/v1`).
...
1. `REVIEWER` — bugs, logic errors, anti-patterns (`ag/claude-sonnet-4-6`)
2. `SECURITY` — OWASP: injection, XSS, auth flaws, secrets (`ag/claude-sonnet-4-6`)
3. `TESTER` — **Adversarial devil's advocate**: tìm những gì 2 reviewer kia BỎ SÓT — race conditions, hidden assumptions, non-obvious edge cases. Mỗi finding bắt buộc kèm input/scenario cụ thể làm code fail. (`ag/gemini-3-flash-agent`)
...
**Pre-pass Summarizer** (khi diff/code > 200KB): MANAGER (`ag/claude-sonnet-4-6`, true 1M context) tóm gọn xuống ~100KB trước khi đưa vào 3 reviewer. Giữ lại: mọi thay đổi security-relevant, logic phân nhánh phức tạp, API/schema thay đổi, dependency imports, tên file + line number chính xác. Bỏ: style/whitespace/comment-only. `fast=True` → bỏ qua pre-pass.

**Stage 2 — Sequential** (`INTEGRITY`, `ag/claude-sonnet-4-6`, 25.77M TPM): nhận code + toàn bộ findings từ Stage 1 làm input, thực hiện 2 việc trong 1 call:
...
| `consult` | Trước khi analyzer (Sonnet) trả lời |
...
ROLE_TIMEOUTS = {
    ...
    "analyzer": _safe_float("ROLE_TIMEOUT_ANALYZER", 300.0),  # sonnet reasoning
    ...
}
```
...
| Gemini 3.5 High | $1.00 | $4.00 |
| Gemini 3.5 Low | $0.15 | $0.60 |
| default | $2.00 | $6.00 |
...
FinOps chỉ thống kê và không chặn 9Router call. `HARNESS_LLM_ENABLED=0` vẫn là kill-switch cứng cho mọi LLM call.

#### Quota reminder cho 9Router

`router_quota_status` là tool read-only để nhắc quota/token còn lại. Tool thử đọc 9Router dashboard `GET /api/usage/stats?period=30d`; nếu cấu hình `HARNESS_ROUTER_QUOTA_CONNECTION_IDS`, nó đọc thêm `GET /api/usage/[connectionId]` để lấy quota theo account/provider khi provider có trả `remaining`/`limit`. Nếu router không chạy hoặc endpoint không có dữ liệu quota thật, tool fallback sang `.harness_finops.db` và ngân sách local.

```env
HARNESS_QUOTA_MONTHLY_TOKENS=10000000
HARNESS_QUOTA_MONTHLY_USD=25
HARNESS_QUOTA_WARN_PCT=80
HARNESS_QUOTA_CRITICAL_PCT=95
HARNESS_ROUTER_QUOTA_CONNECTION_IDS=conn_1,conn_2
# Nếu /api/usage/* trả 401, cấu hình dashboard session/CLI token tương ứng:
HARNESS_ROUTER_QUOTA_BEARER=
HARNESS_ROUTER_QUOTA_COOKIE=
HARNESS_ROUTER_QUOTA_HEADERS_JSON={"X-9Router-CLI-Token":"..."}
```

Kết quả có `warning_level=ok|warn|critical|unknown`, `budget.tokens.remaining`, `budget.usd.remaining`, `router.top_models`, `router.top_accounts`, và `local_finops.model_stats`. Đây là cơ chế nhắc, không phải cost guard: nó không tự block LLM, không tự đổi profile, và chạy được cả khi profile `off` vì chỉ đọc local/router usage.
...
```json
{
  ...
  "total_cache_hits": 38,
  "model_stats": [
    {"model": "ag/claude-sonnet-4-6", "count": 54, "cost_usd": 0.009, "avg_latency_ms": 8200}
  ],
...
### Kỹ thuật 22: Phân Vai 12 Model — 83 MCP Tools, Auto-Pilot + Auto-Watch
...
Hệ thống có **83 MCP tools**: goal autopilot, goal supervisor, direct `goal_runner`, ops/self-check layer, `prod_readiness_gate`, lesson curator, distilled `integration_router`, `hallmark_bridge`, `speckit_bridge`, `scope_creep_detector`, optional `office_bridge`, `router_quota_status`, 7 tool gọi LLM, 55+ static/static-first analyzers, và `auto_trigger` để tự fan-out các tool phù hợp. Mặc định Auto-Pilot chạy static-first với `HARNESS_STATIC_LLM=0` và `HARNESS_AUTO_LLM=0`; scanner vẫn trả kết quả tĩnh, còn 9Router enrichment là opt-in.

| Tool (gọi LLM) | AgentRole | Model 9Router | Mục đích |
|---|---|---|---|
| `ask_codebase` | MANAGER | chain `ag/gemini-3-flash-agent` → `ag/claude-sonnet-4-6` | Q&A xuyên file, fast model chain + local fallback |
| `panel_review` (merge/dedupe) | SYNTHESIZER | `ag/gemini-3-flash-agent` | Gộp findings từ 3 reviewer, fast JSON synthesis |
| `consult` | ANALYZER | `ag/claude-sonnet-4-6` | Kiến trúc + trade-offs, deep reasoning |
| `alt_implementation` (approach A) | CODE_A | `ag/gemini-3-flash-agent` | Implementation proposal code-focused |
| `alt_implementation` (approach B) | CODE_B | `ag/claude-sonnet-4-6` | Implementation khác biệt để so sánh |
| `panel_review` (reviewer/tester/security — 3 song song) | REVIEWER + TESTER + SECURITY | `ag/gemini-3-flash-agent` / `ag/gemini-3-flash-agent` / `ag/claude-sonnet-4-6` | Code quality + adversarial + OWASP |
| `suggest_fix`, `security_autofix` | DEBUGGER | `ag/gemini-3-flash-agent` | Root cause + unified diff patch |
| `quick_task` | WORKER | `ag/gemini-3.5-flash-extra-low` | Boilerplate, fixtures, docstring |
...
`lesson_curator` là lớp phân loại/lọc lesson trước khi promote global. Auto append dùng static curator nhanh để gắn `curation` và chỉ tự promote lesson dạng procedure/workflow reusable từ trusted source (`procedure`, `goal_runner`, `goal_runner_fallback`, `mcp_tool_fallback`), không promote `edit_event`, `project_seen`, `checked_edit`, `panel_review`, traceback/timeout, `.env`, local path, secret-like text. MCP tool `lesson_curator(mode="max")` dùng 9Router 3-agent adjudication (`tester` = `ag/gemini-3-flash-agent`, `security` = `ag/claude-sonnet-4-6`, `reviewer` = `ag/claude-sonnet-4-6`) cho các candidate/mơ hồ: mỗi agent trả JSON vote, chỉ promote global khi đa số đồng ý `global_procedure` và source qua final trusted-source gate; muốn import/promote nguồn manual phải bật rõ `allow_untrusted_promote=true`. `mode="safe"` chỉ chạy static rule để audit nhanh không gọi 9Router. Benchmark local trên 3 case curate bắt buộc cho thấy bộ này đúng 3/3 mỗi model với ~1.3-2.3s/call; old pro candidate `ag/claude-sonnet-4-6` timeout 3/3 ở 20s nên không dùng cho curator mặc định.
