...
Hệ thống Agent Harness được xây dựng để làm "hội đồng cố vấn" hỗ trợ cho AI coder chính (**Claude Code**, **Gemini** trên Antigravity IDE). Nó cung cấp **61 MCP tools** tích hợp trực tiếp thông qua 12 AI agents chạy trên Azure AI Foundry; Auto-Pilot có thể tự gọi các scanner/reviewer phù hợp sau edit.
...
### 2.6. Danh mục đủ 61 MCP tools và cần chuẩn bị gì

| Nhóm | Tools | Phụ thuộc chính | Khi dùng |
|---|---|---|---|
| Orchestration | `goal_autopilot`, `goal_supervisor`, `auto_trigger`, `prod_readiness_gate`, `run_single_agent`, `list_agents` | MCP connected, `.env` nếu gọi agent LLM | Prompt-only goal flow, hard next-action loop, Auto-Pilot, production gate, gọi thẳng agent, xem model mapping |
...
Kết quả: tất cả pass. `smoke_test.py` xác nhận MCP registry có 61 tools, `goal_autopilot status`, `goal_supervisor`, và các nhánh `prod_readiness_gate` mới:
...
`mode="safe"` nhẹ hơn, hợp smoke/offline checks, nhưng không đủ để kết luận production-ready cho release thật. `mode="max"` bật full pre-prod gate: final Auto-Pilot, config/env/secret, panel review, SBOM, breaking change, OpenAPI/migration/container/CI checks theo context.
...
Nếu `ask_codebase` vẫn timeout 300s trong client hiện tại, gần như chắc là MCP process cũ chưa restart. Chạy lại client hoặc restart MCP server, rồi gọi `goal_supervisor`/`list_tools` để xác nhận thấy 61 tools.
<!-- /harness-current-operations -->
...
|---|---|---|
| MCP server | Cho agent chính gọi được 61 tools | `mcp_server.py` qua MCP config |
...
Cursor/Windsurf/generic MCP clients dùng cùng server command/path ở trên. Nếu một agent chính không hỗ trợ MCP thì không gọi trực tiếp được 61 tools; cần wrapper/bridge riêng.

#### Memory/rules tối thiểu cho agent khác

Không cần copy schema của 61 tools vào rules file. Tool schema nằm trong `mcp_server.py -> list_tools()`. Rules chỉ cần policy gọi tool:
...
Nếu agent chính không support MCP, nó không thể gọi trực tiếp 61 tools. Có 3 hướng:
...
### Kỹ thuật 22: Phân Vai 12 Model — 61 MCP Tools, Auto-Pilot + Auto-Watch
...
Hệ thống có **61 MCP tools**: goal autopilot, goal supervisor, `prod_readiness_gate`, 7 tool gọi LLM, 50 static analyzers, và `auto_trigger` để tự fan-out các tool phù hợp. Mặc định Auto-Pilot bật `HARNESS_STATIC_LLM=1`, nên các static scanner vẫn chạy phân tích tĩnh trước rồi gọi Azure để enrich/triage khi tool hỗ trợ.
