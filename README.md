...
Hệ thống Agent Harness được xây dựng để làm "hội đồng cố vấn" hỗ trợ cho AI coder chính (**Claude Code**, **Gemini** trên Antigravity IDE). Runtime hiện đăng ký **77 MCP tools**: core LLM tools, goal autopilot, lesson memory và static/static-first analyzers; Auto-Pilot có thể tự gọi scanner/reviewer phù hợp sau edit.
...
| FinOps | SQLite file `.harness_finops.db` tự tạo | `finops_stats`; chỉ thống kê token/cost, không chặn request |
| Web dashboard tùy chọn | `python server.py` | Mở `http://localhost:8000` |

### 2.6. Danh mục đủ 77 MCP tools và cần chuẩn bị gì
...
Kết quả: tất cả pass. `smoke_test.py` xác nhận MCP registry có 77 tools, `goal_autopilot status`, `goal_supervisor`, `goal_runner` dry-run, ops tools, 5 gap tools mới, và các nhánh `prod_readiness_gate`:
...
|---|---|---|
| MCP server | Cho agent chính gọi được 77 tools | `mcp_server.py` qua MCP config |
...
Cursor/Windsurf/generic MCP clients dùng cùng server command/path ở trên. Nếu một agent chính không hỗ trợ MCP thì không gọi trực tiếp được 77 tools; cần wrapper/bridge riêng.

#### Memory/rules tối thiểu cho agent khác

Không cần copy schema của 77 tools vào rules file. Tool schema nằm trong `mcp_server.py -> list_tools()`. Rules chỉ cần policy gọi tool:
...
Nếu agent chính không support MCP, nó không thể gọi trực tiếp 77 tools. Có 3 hướng:
...
### Kỹ thuật 22: Phân Vai 12 Model — 77 MCP Tools, Auto-Pilot + Auto-Watch
...
Hệ thống có **77 MCP tools**: goal autopilot, goal supervisor, direct `goal_runner`, ops/self-check layer, `prod_readiness_gate`, lesson curator, 7 tool gọi LLM, 55 static/static-first analyzers, và `auto_trigger` để tự fan-out các tool phù hợp. Mặc định Auto-Pilot chạy static-first với `HARNESS_STATIC_LLM=0` và `HARNESS_AUTO_LLM=0`; scanner vẫn trả kết quả tĩnh, còn 9Router enrichment là opt-in.
