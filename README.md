...
| :--- | :--- | :--- |
| **Manager** | `gpt-5.4-pro-3` | Agent manager mặc định cho tác vụ long-context. Riêng `ask_codebase` dùng fast model chain `gpt-5.4-4 → gpt-5.4-3 → gpt-5.3-codex-4` + fallback local có `file:line`. |
| **Synthesizer** | `gpt-5.4-4` | Fast JSON synthesis/merge cho `panel_review`; benchmark nhanh hơn `gpt-5.4-pro-2` và không timeout trên merge nhỏ. |
| **Analyzer** | `grok-4-20-reasoning` | Đề xuất giải pháp kiến trúc và phân tích trade-offs (concurrency, performance). |
| **Code A** | `gpt-5.3-codex` | Code agent thứ nhất, code-focused, nhanh và ổn định cho implementation proposal. |
...
Benchmark role ngày 2026-07-14 trên task đại diện: 11/12 role cũ pass, riêng `gpt-5.4-pro-2` cho Synthesizer timeout 45s ở task merge nhỏ; candidate `gpt-5.4-4` pass ~2.9s. `Kimi-K2.6` cho Code A chậm/không ổn định trên smoke codegen; `gpt-5.3-codex` pass ~2.2s và hợp vai code-focused hơn. Vì vậy default ưu tiên model nhanh/đúng; pro chỉ nên dùng khi có nhu cầu long-context đặc biệt và được override bằng env.
...
MODEL_SYNTHESIZER=gpt-5.4-4
MODEL_ANALYZER=grok-4-20-reasoning
MODEL_CODE_A=gpt-5.3-codex
...
SPARE_MODELS=gpt-5.4-4,gpt-5.4-3,gpt-5.3-codex-4,gpt-5.4,gpt-5.4-2,gpt-4.1-mini
HARNESS_KNOWN_DEPLOYMENTS=gpt-4.1-mini

# ask_codebase fast path.
HARNESS_ASK_CODEBASE_MODEL_CHAIN=gpt-5.4-4,gpt-5.4-3,gpt-5.3-codex-4
...
| Goal Autopilot | `python merge_settings.py` + MCP connected | Prompt một lần; `goal_autopilot(init)` chia parts, `auto_trigger(mode=max)` check từng part, `goal_supervisor` trả next action cứng |
...
- docs-only safe path trả verdict hợp lệ, không cần Azure.
...
- **Contextual tool coverage:** `auto_trigger(mode=max)` không còn chỉ quanh 6-10 tool mặc định; nó tự mở rộng theo signal DB/API/UI/CI/container/dependency/test/perf, rồi tự ưu tiên nhóm security/config/devops/release/panel trước khi chạy.
...
- `HARNESS_ASK_CODEBASE_MODEL_CHAIN` là thứ tự Azure model thử cho `ask_codebase`; default tránh `gpt-5.4-pro-3` vì probe thực tế hay timeout với context lớn. Có thể override bằng `HARNESS_ASK_CODEBASE_MODEL_CHAIN` hoặc `HARNESS_ASK_CODEBASE_MODEL`.
- `USE_SPARES=0` để không treo nhiều phút khi Azure Manager đuối; fail fast sang fallback local.
- `LOAD_BYTES` là lượng context đọc rộng ban đầu.
- `CONTEXT_BYTES` là cap sau prune trước khi gửi Azure; default 220KB để Manager đỡ timeout, vẫn có thể tăng khi cần reasoning sâu hơn.
...
- Nếu thật sự muốn Azure cố hơn, tăng `TIMEOUT=60` hoặc bật `USE_SPARES=1`, nhưng sẽ chậm hơn.

Output tốt phải có một trong hai dạng:

- Azure answer có citation `file:line`.
...
Nếu `ask_codebase` vẫn báo model cũ như `gpt-5.4-pro-3` và không có `model_attempts`, MCP process hiện tại chưa nạp bản mới. Sau lần restart/nạp mới, `mcp_server.py` tự hot-reload `support_tools` + `tools/*` khi file đổi, nên các update tiếp theo không cần chạy lại settings thủ công.
<!-- /harness-current-operations -->
...
| `panel_review`/`ask_codebase` timeout hoặc answer bị cụt | Azure quota thấp, context quá lớn, output budget thấp, primary model chậm, hoặc MCP process cũ chưa restart | Với `ask_codebase`, restart MCP client trước; giữ spares tắt mặc định để fallback nhanh; tăng `HARNESS_ASK_CODEBASE_MAX_OUTPUT_TOKENS` cho câu hỏi rất rộng; chỉ tăng `HARNESS_ASK_CODEBASE_CONTEXT_BYTES` hoặc bật `HARNESS_ASK_CODEBASE_USE_SPARES=1` khi thật sự cần |
...
| `auto_trigger(max)` timeout ở MCP client | Quá nhiều check Azure chạy cùng lúc hoặc tool con không cancel nhanh | Per-tool timeout bị cap 240s; tổng call cap 270s; mặc định chỉ chạy top-priority checks trong batch và trả `degraded/skipped_tools` nếu phải defer |
| `panel_review` kẹt stage synthesize | Synthesizer model timeout/retry hoặc context quá lớn | Synthesizer mặc định dùng `gpt-5.4-4` thay vì pro; panel reviewer tắt retry/spare nội bộ; Integrity synthesize cap bằng `HARNESS_PANEL_INTEGRITY_TIMEOUT` (default 75s), fail thì trả `degraded: true` + local dedup |
...
2. `SECURITY` — OWASP: injection, XSS, auth flaws, secrets (`gpt-5.3-codex-3`)
3. `TESTER` — **Adversarial devil's advocate**: tìm những gì 2 reviewer kia BỎ SÓT — race conditions, hidden assumptions, non-obvious edge cases. Mỗi finding bắt buộc kèm input/scenario cụ thể làm code fail. (`gpt-5.3-codex-2`)
...
Hệ thống có **77 MCP tools**: goal autopilot, goal supervisor, direct `goal_runner`, ops/self-check layer, `prod_readiness_gate`, lesson curator, 7 tool gọi LLM, 55 static/static-first analyzers, và `auto_trigger` để tự fan-out các tool phù hợp. Mặc định Auto-Pilot bật `HARNESS_STATIC_LLM=1`, nên các static scanner vẫn chạy phân tích tĩnh trước rồi gọi Azure để enrich/triage khi tool hỗ trợ.

| Tool (gọi LLM) | AgentRole | Model Azure | Mục đích |
|---|---|---|---|
| `ask_codebase` | MANAGER | chain `gpt-5.4-4` → `gpt-5.4-3` → `gpt-5.3-codex-4` | Q&A xuyên file, fast model chain + local fallback |
| `panel_review` (merge/dedupe) | SYNTHESIZER | `gpt-5.4-4` | Gộp findings từ 3 reviewer, fast JSON synthesis |
| `consult` | ANALYZER | `grok-4-20-reasoning` | Kiến trúc + trade-offs, deep reasoning |
| `alt_implementation` (approach A) | CODE_A | `gpt-5.3-codex` | Implementation proposal code-focused |
...
Các tool nền vẫn chạy Python/subprocess trước. Riêng 5 gap tools mới sẽ gọi Azure enrichment khi `mode="max"` hoặc `HARNESS_STATIC_LLM=1`; smoke/offline dùng `mode="safe"` để bỏ qua Azure.
...
`lesson_curator` là lớp phân loại/lọc lesson trước khi promote global. Auto append dùng static curator nhanh để gắn `curation` và chỉ tự promote lesson dạng procedure/workflow reusable từ trusted source (`procedure`, `goal_runner`, `goal_runner_fallback`, `mcp_tool_fallback`), không promote `edit_event`, `project_seen`, `checked_edit`, `panel_review`, traceback/timeout, `.env`, local path, secret-like text. MCP tool `lesson_curator(mode="max")` dùng Azure 3-agent adjudication (`tester` = `gpt-5.3-codex-2`, `security` = `gpt-5.3-codex-3`, `reviewer` = `gpt-5.3-codex`) cho các candidate/mơ hồ: mỗi agent trả JSON vote, chỉ promote global khi đa số đồng ý `global_procedure` và source qua final trusted-source gate; muốn import/promote nguồn manual phải bật rõ `allow_untrusted_promote=true`. `mode="safe"` chỉ chạy static rule để audit nhanh không gọi Azure. Benchmark local trên 3 case curate bắt buộc cho thấy bộ này đúng 3/3 mỗi model với ~1.3-2.3s/call; old pro candidate `gpt-5.4-pro-2` timeout 3/3 ở 20s nên không dùng cho curator mặc định.
