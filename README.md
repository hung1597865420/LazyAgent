...
Hệ thống Agent Harness được xây dựng để làm "hội đồng cố vấn" hỗ trợ cho AI coder chính (**Claude Code**, **Gemini** trên Antigravity IDE). Nó cung cấp **76 MCP tools** tích hợp trực tiếp thông qua 12 AI agents chạy trên Azure AI Foundry; Auto-Pilot có thể tự gọi các scanner/reviewer phù hợp sau edit.
...
### 2.6. Danh mục đủ 76 MCP tools và cần chuẩn bị gì
...
Kết quả: tất cả pass. `smoke_test.py` xác nhận MCP registry có 76 tools, `goal_autopilot status`, `goal_supervisor`, `goal_runner` dry-run, ops tools, 5 gap tools mới, và các nhánh `prod_readiness_gate`:
...
|---|---|---|
| MCP server | Cho agent chính gọi được 76 tools | `mcp_server.py` qua MCP config |
...
Cursor/Windsurf/generic MCP clients dùng cùng server command/path ở trên. Nếu một agent chính không hỗ trợ MCP thì không gọi trực tiếp được 76 tools; cần wrapper/bridge riêng.

#### Memory/rules tối thiểu cho agent khác

Không cần copy schema của 76 tools vào rules file. Tool schema nằm trong `mcp_server.py -> list_tools()`. Rules chỉ cần policy gọi tool:
...
Nếu agent chính không support MCP, nó không thể gọi trực tiếp 76 tools. Có 3 hướng:
...
### Kỹ thuật 22: Phân Vai 12 Model — 76 MCP Tools, Auto-Pilot + Auto-Watch
...
Hệ thống có **76 MCP tools**: goal autopilot, goal supervisor, direct `goal_runner`, ops/self-check layer, `prod_readiness_gate`, 7 tool gọi LLM, 55 static/static-first analyzers, và `auto_trigger` để tự fan-out các tool phù hợp. Mặc định Auto-Pilot bật `HARNESS_STATIC_LLM=1`, nên các static scanner vẫn chạy phân tích tĩnh trước rồi gọi Azure để enrich/triage khi tool hỗ trợ.
