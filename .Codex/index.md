...
- harness.install.json — Static setup manifest for dry-run install profiles and agent target checks.
...
- tools/integrations.py — Static Hallmark/Spec Kit/UI Skills router plus guarded `hallmark_bridge`, `speckit_bridge`, and `ui_skill_router` tools.
- tools/workflow.py — Static Matt-skills workflow router and bug repro guard for debug/spec/domain/review/TDD/architecture flows.
...
- tools/ops.py — Harness ops tools for doctor, install manifest, adapter parity, MCP inventory, context budget, ledger, profiles, and patch safety.
